...
let mapStore = useMapStore(currStore.currStoreId)()
let optionStr = ref(mapStore.option)
//zindex属性在父组件上
...
function updateComp(e) {
    mapStore = useMapStore(currStore.currStoreId)()
    mapStore.$patch({
        attribute: [
            {
                name: "宽度",
...
function downloadData() {
    mapStore = useMapStore(currStore.currStoreId)()
    let dataStr = JSON.stringify(mapStore.option)
    //调用导出json数据方法
    exportFileJSON(dataStr, `${currStore.currStoreId}.json`)
}

//设置静态数据与动态请求
let { sandShow } = toRefs(mapStore)
let { ajaxMethod } = toRefs(mapStore)
let selectDataSource = ['静态数据', '动态请求']
let selectMethods = ['get', 'post']
let { ajaxUrl } = toRefs(mapStore)
let { currDataSource } = toRefs(mapStore)
let { timeout } = toRefs(mapStore)
    ...
    if (timeout.value == undefined) {
        ...
        const promise = new Promise(async (resolve, reject) => {
            ...
            timer.value = setInterval(async () => {
                ...
                mapStore.importOption(jsonData)
