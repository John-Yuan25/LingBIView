...
//设置静态数据与动态请求
let { sandShow } = toRefs(pieStore)
let { ajaxMethod } = toRefs(pieStore)
let selectDataSource = ['静态数据', '动态请求']
let selectMethods = ['get', 'post']
let { ajaxUrl } = toRefs(pieStore)
let { currDataSource } = toRefs(pieStore)
let { timeout } = toRefs(pieStore)
    ...
    if (timeout.value == undefined) {
        ...
        const promise = new Promise(async (resolve, reject) => {
            ...
            timer.value = setInterval(async () => {
                ...
                pieStore.importOption(jsonData)
