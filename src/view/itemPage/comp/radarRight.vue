...
import { useRadarStore, useCurrStore } from '@/stores'
...
let radarStore = useRadarStore(currStore.currStoreId)()
let optionStr = ref(radarStore.option)
//zindex属性在父组件上
const parentNode = document.getElementById(currStore.currStoreId) as HTMLElement;

//监听图表的数据变化,更新optionStr
watch(() => radarStore.option, (newValue, oldValue) => {
...
function updateComp(e) {
    radarStore = useRadarStore(currStore.currStoreId)()
    radarStore.$patch({
        attribute: [
            {
                name: "宽度",
...
function downloadData() {
    radarStore = useRadarStore(currStore.currStoreId)()
    let dataStr = JSON.stringify(radarStore.option)
    //调用导出json数据方法
    exportFileJSON(dataStr, `${currStore.currStoreId}.json`)
}

//设置静态数据与动态请求
let { sandShow } = toRefs(radarStore)
let { ajaxMethod } = toRefs(radarStore)
let selectDataSource = ['静态数据', '动态请求']
let selectMethods = ['get', 'post']
let { ajaxUrl } = toRefs(radarStore)
let { currDataSource } = toRefs(radarStore)
let { timeout } = toRefs(radarStore)
    ...
    if (timeout.value == undefined) {
        ...
        const promise = new Promise(async (resolve, reject) => {
            ...
            timer.value = setInterval(async () => {
                ...
                radarStore.importOption(jsonData)
