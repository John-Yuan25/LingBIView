...
watch(() => categoryLineStore.option.dataset, (newValue, oldValue) => {
...
function updateComp(e) {
    categoryLineStore = useCategoryLineStore(currStore.currStoreId)()
    categoryLineStore.$patch({
        attribute: [
            {
                name: "宽度",
                type: 'number',
...
function importData(e) {
    ...
    importFileJSON(e).then((res) => {
        //调用actions的方法
        categoryLineStore.importOption(res)
    }).catch((e) => {
        alert(e)
    })
}
...
function downloadData() {
    categoryLineStore = useCategoryLineStore(currStore.currStoreId)()
    let dataStr = JSON.stringify(categoryLineStore.option.dataset)
    //调用导出json数据方法
    ...
    if (timeout.value == undefined) {
        ...
        const promise = new Promise(async (resolve, reject) => {
            ...
            timer.value = setInterval(async () => {
                ...
                categoryLineStore.importOption(jsonData)
