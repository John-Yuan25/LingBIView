  ...
  <div class="wrapper">
    ...
    <div v-show="props.checkIndex === 0">
      <div class="styleItem" v-for="(item, index) in thiscurrComp.attribute" :key="index">
        ...
        <input class="inputStyle" v-if="item.type === 'number'" type="number" v-model="item.value" @change="updateComp">
        <input class="inputStyle" v-if="item.type === 'color'" type="color" v-model="item.value" @change="updateComp">
      </div>
    </div>
    <!-- 数据 -->
    <div v-show="props.checkIndex === 1">
      <div class="styleItem">
        ...
        <input class="inputStyle" type="number" v-model="BDmapStore.pointE">
      </div>
      <div class="styleItem">
        <span class="label">纬度:</span>
        <input class="inputStyle" type="number" v-model="BDmapStore.pointN">
      </div>
      <div class="styleItem">
        <span class="label">标注经度:</span>
        <input class="inputStyle" type="number" v-model="BDmapStore.markerPointE">
      </div>
      <div class="styleItem">
        <span class="label">标注纬度:</span>
        <input class="inputStyle" type="number" v-model="BDmapStore.markerPointN">
      </div>
    </div>
    <!-- 编辑事件 -->
...
let BDmapStore = useBDmapStore(currStore.currStoreId)()
...
function updateComp(e) {
  BDmapStore = useBDmapStore(currStore.currStoreId)()
  BDmapStore.$patch({
    attribute: [
      {
        name: "宽度",
...
const openEditor = function () {
  currStore.editorCode = BDmapStore.code;
