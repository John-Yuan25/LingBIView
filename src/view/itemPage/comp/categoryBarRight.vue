...
import { useCategoryBarStore, useCurrStore } from '@/stores'
...
let optionStr = ref(categoryBarStore.option)
//zindex属性在父组件上
...
function updateComp(e) {
    categoryBarStore = useCategoryBarStore(currStore.currStoreId)()
    categoryBarStore.$patch({
        attribute: [
            {
                name: "宽度",
                type: 'number',
...
function importData(e) {
    ...
    importFileJSON(e).then((res) => {
        //调用actions的方法
        categoryBarStore.importOption(res)
    }).catch((e) => {
        alert(e)
    })
}
...
function downloadData() {
    categoryBarStore = useCategoryBarStore(currStore.currStoreId)()
    let dataStr = JSON.stringify(categoryBarStore.option)
    //调用导出json数据方法
    exportFileJSON(dataStr, `${currStore.currStoreId}.json`)
}

//设置静态数据与动态请求
let { sandShow } = toRefs(categoryBarStore)
let { ajaxMethod } = toRefs(categoryBarStore)
let selectDataSource = ['静态数据', '动态请求']
let selectMethods = ['get', 'post']
let { ajaxUrl } = toRefs(categoryBarStore)
let { currDataSource } = toRefs(categoryBarStore)
let { timeout } = toRefs(categoryBarStore)
...
//发送请求
let { timer } = toRefs(categoryBarStore)
    ...
    if (timeout.value == undefined) {
        ...
        const promise = new Promise(async (resolve, reject) => {
            ...
            timer.value = setInterval(async () => {
                ...
                categoryBarStore.importOption(jsonData)
