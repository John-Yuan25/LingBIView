...
import { useGaugeStore, useCurrStore } from '@/stores'
...
let gaugeStore = useGaugeStore(currStore.currStoreId)()
let optionStr = ref(gaugeStore.option)
//zindex属性在父组件上
const parentNode = document.getElementById(currStore.currStoreId) as HTMLElement;

//监听图表的数据变化,更新optionStr
watch(() => gaugeStore.option, (newValue, oldValue) => {
...
function updateComp(e) {
    gaugeStore = useGaugeStore(currStore.currStoreId)()
    gaugeStore.$patch({
        attribute: [
            {
                name: "宽度",
...
function downloadData() {
    gaugeStore = useGaugeStore(currStore.currStoreId)()
    let dataStr = JSON.stringify(gaugeStore.option)
    //调用导出json数据方法
    exportFileJSON(dataStr, `${currStore.currStoreId}.json`)
}

//设置静态数据与动态请求
let { sandShow } = toRefs(gaugeStore)
let { ajaxMethod } = toRefs(gaugeStore)
let selectDataSource = ['静态数据', '动态请求']
let selectMethods = ['get', 'post']
let { ajaxUrl } = toRefs(gaugeStore)
let { currDataSource } = toRefs(gaugeStore)
let { timeout } = toRefs(gaugeStore)
    ...
    if (timeout.value == undefined) {
        ...
        const promise = new Promise(async (resolve, reject) => {
            ...
            timer.value = setInterval(async () => {
                ...
                gaugeStore.importOption(jsonData)
