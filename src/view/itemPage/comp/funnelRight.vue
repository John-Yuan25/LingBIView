...
let funnelStore = useFunnelStore(currStore.currStoreId)()
let optionStr = ref(funnelStore.option)
//zindex属性在父组件上
const parentNode = document.getElementById(currStore.currStoreId) as HTMLElement;

//监听图表的数据变化,更新optionStr
watch(() => funnelStore.option, (newValue, oldValue) => {
...
function updateComp(e) {
    funnelStore = useFunnelStore(currStore.currStoreId)()
    funnelStore.$patch({
        attribute: [
            {
                name: "宽度",
...
function downloadData() {
    funnelStore = useFunnelStore(currStore.currStoreId)()
    let dataStr = JSON.stringify(funnelStore.option)
    //调用导出json数据方法
    exportFileJSON(dataStr, `${currStore.currStoreId}.json`)
}

//设置静态数据与动态请求
let { sandShow } = toRefs(funnelStore)
let { ajaxMethod } = toRefs(funnelStore)
let selectDataSource = ['静态数据', '动态请求']
let selectMethods = ['get', 'post']
let { ajaxUrl } = toRefs(funnelStore)
let { currDataSource } = toRefs(funnelStore)
let { timeout } = toRefs(funnelStore)
    ...
    if (timeout.value == undefined) {
        ...
        const promise = new Promise(async (resolve, reject) => {
            ...
            timer.value = setInterval(async () => {
                ...
                funnelStore.importOption(jsonData)
