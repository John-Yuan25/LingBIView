...
let kLineStore = useKLineStore(currStore.currStoreId)()
let optionStr = ref(kLineStore.option)
//zindex属性在父组件上
const parentNode = document.getElementById(currStore.currStoreId) as HTMLElement;

//监听图表的数据变化,更新optionStr
watch(() => kLineStore.option, (newValue, oldValue) => {
...
function updateComp(e) {
    kLineStore = useKLineStore(currStore.currStoreId)()
    kLineStore.$patch({
        attribute: [
            {
                name: "宽度",
...
function downloadData() {
    kLineStore = useKLineStore(currStore.currStoreId)()
    let dataStr = JSON.stringify(kLineStore.option)
    //调用导出json数据方法
    exportFileJSON(dataStr, `${currStore.currStoreId}.json`)
}

//设置静态数据与动态请求
let { sandShow } = toRefs(kLineStore)
let { ajaxMethod } = toRefs(kLineStore)
let selectDataSource = ['静态数据', '动态请求']
let selectMethods = ['get', 'post']
let { ajaxUrl } = toRefs(kLineStore)
let { currDataSource } = toRefs(kLineStore)
let { timeout } = toRefs(kLineStore)
    ...
    if (timeout.value == undefined) {
        ...
        const promise = new Promise(async (resolve, reject) => {
            ...
            timer.value = setInterval(async () => {
                ...
                kLineStore.importOption(jsonData)
