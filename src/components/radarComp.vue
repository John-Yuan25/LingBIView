...
const radarStore = useRadarStore(storeId)()
// let styleAttribute: any = toRef(pieStore, 'attribute')
let { attribute } :any= toRefs(radarStore)
...
option = radarStore.option as EChartsOption
onMounted(() => {
...
//监听图表的数据变化
watch(() => radarStore.option.series[0].data, (newValue, oldValue) => {
    //更新图表数据
    option = radarStore.option as EChartsOption
    option && myChart.setOption(option);
}
