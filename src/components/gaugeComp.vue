...
const gaugeStore = useGaugeStore(storeId)()
// let styleAttribute: any = toRef(pieStore, 'attribute')
let { attribute } :any= toRefs(gaugeStore)
...
option = gaugeStore.option as EChartsOption
onMounted(() => {
...
//监听图表的数据变化
watch(() => gaugeStore.option.series[0].data, (newValue, oldValue) => {
    //更新图表数据
    option = gaugeStore.option as EChartsOption
    option && myChart.setOption(option);
}
